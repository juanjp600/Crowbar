...
#Region "Methods"
			...
			If Me.theMdlFileData.theBoneControllers IsNot Nothing Then
				...
				For i As Integer = 0 To Me.theMdlFileData.theBoneControllers.Count - 1
					...
					If boneController.index = 4 Then
						line += "Mouth"
					Else
						line += boneController.index.ToString(AppConstants.InternalNumberFormat)
					End If
					line += " """
	...
	Public Sub WriteFlagsCommand()
		...
		Else
			line = "$flags "
		End If
		line += Me.theMdlFileData.flags.ToString(AppConstants.InternalNumberFormat)
		Me.theOutputFileStreamWriter.WriteLine(line)
	End Sub
		...
		If Me.theMdlFileData.theSequenceGroups.Count > 1 Then
			...
			If largestFileSize > 0 Then
				...
				Else
					line = "$sequencegroupsize "
				End If
				line += groupSize.ToString(AppConstants.InternalNumberFormat)
	...
	Public Sub WriteTexRenderMode()
		If Me.theMdlFileData.theTextures IsNot Nothing AndAlso Me.theMdlFileData.theTextures.Count > 0 Then
			...
			For Each texture As SourceMdlTexture14 In Me.theMdlFileData.theTextures
				If ((texture.flags And SourceMdlTexture14.STUDIO_NF_FLATSHADE) > 0) AndAlso (Not texture.theFileName.ToLowerInvariant().Contains("chrome")) Then
					Me.WriteTexRenderModeLine(texture.theFileName, "flatshade", True)
				End If
				If ((texture.flags And SourceMdlTexture14.STUDIO_NF_CHROME) > 0) AndAlso (Not texture.theFileName.ToLowerInvariant().Contains("chrome")) Then
...
#Region "Private Methods"
	...
	Private Sub WriteSequenceOptions(ByVal aSequenceDesc As SourceMdlSequenceDesc10)
		...
		If aSequenceDesc.activityId > 0 Then
			...
			Else
				activityName = "ACT_" + aSequenceDesc.activityId.ToString(AppConstants.InternalNumberFormat)
			End If
			line = vbTab
			line += activityName
			line += " "
		...
		If aSequenceDesc.thePivots IsNot Nothing AndAlso aSequenceDesc.thePivots.Count > 0 Then
			For pivotIndex As Integer = 0 To aSequenceDesc.thePivots.Count - 1
				line = vbTab
				line += "pivot "
				line += pivotIndex.ToString(AppConstants.InternalNumberFormat)
				line += " "
				line += aSequenceDesc.thePivots(0).pivotStart.ToString(AppConstants.InternalNumberFormat)
				line += " "
				line += aSequenceDesc.thePivots(0).pivotEnd.ToString(AppConstants.InternalNumberFormat)
				Me.theOutputFileStreamWriter.WriteLine(line)
			Next
		End If
