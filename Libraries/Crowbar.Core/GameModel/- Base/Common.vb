...
Module Common
		...
		If flexRule.theFlexOps IsNot Nothing AndAlso flexRule.theFlexOps.Count > 0 Then
			...
			For i As Integer = 0 To flexRule.theFlexOps.Count - 1
				...
				ElseIf aFlexOp.op = SourceMdlFlexOp.STUDIO_DME_LOWER_EYELID Then
					...
					Dim flCloseLidVMin As String = Math.Round(pCloseLidV.min, 6).ToString("0.######", AppConstants.InternalNumberFormat)
					Dim flCloseLidVMax As String = Math.Round(pCloseLidV.max, 6).ToString("0.######", AppConstants.InternalNumberFormat)
					...
					Dim flCloseLidMin As String = Math.Round(pCloseLid.min, 6).ToString("0.######", AppConstants.InternalNumberFormat)
					Dim flCloseLidMax As String = Math.Round(pCloseLid.max, 6).ToString("0.######", AppConstants.InternalNumberFormat)
					...
					Dim flEyeUpDownMin As String = Math.Round(pEyeUpDown.min, 6).ToString("0.######", AppConstants.InternalNumberFormat)
					Dim flEyeUpDownMax As String = Math.Round(pEyeUpDown.max, 6).ToString("0.######", AppConstants.InternalNumberFormat)
	...
	Public Sub ProcessTexturePaths(ByVal theTexturePaths As List(Of String), ByVal theTextures As List(Of SourceMdlTexture), ByVal theModifiedTexturePaths As List(Of String), ByVal theModifiedTextureFileNames As List(Of String))
		...
		If AppSettings.Instance.DecompileRemovePathFromSmdMaterialFileNamesIsChecked Then
	...
	Public Sub WriteHeaderComment(ByVal outputFileStreamWriter As StreamWriter)
		If Not AppSettings.Instance.DecompileStricterFormatIsChecked Then
			Dim line As String = ""

			line = "// "
			line += AppConstants.GetHeaderComment()
